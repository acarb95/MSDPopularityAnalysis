...
public class SongSortReducer extends Reducer<Text, SongWritable, Text, Text> {
	...
	public void reduce(Text key, Iterable<SongWritable> values, Context context) throws IOException, InterruptedException {
		...
		for (int i = 0; i < length; i++) {
			mos.write(key, new Text(list.get(i).title + "\t" + list.get(i).outputFeatures()), key + "SortedSongs");
		}
	}
