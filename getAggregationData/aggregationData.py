...
class NgramNeighbors(MRJob):
	...
	def mapper(self, key, line):
		...
		featureArray.extend(getAggregations(normalizeArray(np.fromstring(data[6].strip("[]")),  sep=', '))) #barsStart
		featureArray.extend(getAggregations(normalizeArray(np.fromstring(data[7].strip("[]")),  sep=', '))) #beatsStart
		featureArray.extend(getAggregations(normalizeArray(np.fromstring(data[8].strip("[]")),  sep=', '))) #sectionsStart
		featureArray.extend(getAggregations(normalizeArray(np.fromstring(data[9].strip("[]")),  sep=', '))) #segmentsMaxLoudness
		featureArray.extend(getAggregations(normalizeArray(np.fromstring(data[10].strip("[]")),  sep=', '))) #segmentsMaxLoudnessTime
		featureArray.extend(getAggregations(normalizeArray(np.fromstring(data[11].strip("[]")),  sep=', '))) #segmentsMaxLoudnessStart
		featureArray.extend(getAggregations(normalizeArray(np.fromstring(data[12].strip("[]")),  sep=', '))) #segmentsStart
		featureArray.extend(getAggregations(normalizeArray(np.fromstring(data[13].strip("[]")),  sep=', '))) #tatumsStart
		# 2D arrays
		timbre = data[14]
		pitch = data[15]
		...
		for item in timbreSplit:
			...
			featureArray.extend(getAggregations(normalizeArray(item)))
