...
/**
 * Modifications:
 * 	HDF5GettersV2 taken from this website: http://www.hadoopinrealworld.com/using-million-song-dataset-in-hadoop/
 * 	Contains this added functionality: 
 * 		Traverses folders recursively
 * 		Grabs song metadata and prints it (tab-delimited) to a file
 * 		Modified main to remove printing code and to call new utility functions
 * 	My Modifications:
 * 		Instead of printing "" when NaN is encountered, I print "NaN" (maintains index numbers)
 * 		Modified browsefolders and main to create one file for each subfolder that contains files (i.e. if the input directory is the entire dataset it will create files for each folder that contains the tracks data-A-A-A)
 * 
 * @author acarbona
 * @date 4/27/15
 */
...
public class HDF5GettersV3 {
	...
	public static void extractSongInfo(String filename, FileWriter fw) {
		...
		// Attempt to read in all the data then print it out tab delimited
		try {
			// metadata
			...
			fw.write(latlong.equals("NaN") ? "NaN\t" : latlong + "\t");
			...
			fw.write(hot.equals("NaN") ? "NaN\t" : hot + "\t");
			...
			// Suppress errors (some track files are empty or unreadable)
		}
		hdf5_close(h5);
	}

	private static String orig_foldername;
	
	public static void browseFolder(String input_foldername, String output_foldername) throws IOException {
		// Create the folder object and list of files. 
		File folder = new File(input_foldername);
		File[] listOfFiles = folder.listFiles();
		
		// Get the input folder name and remove the original foldername
		// i.e. /s/bach/k/under/acarbona/MSD_data/data/A/A/A --> /data/A/A/A
		String name = input_foldername.replace(orig_foldername, "");
		
		// Replace all slashes with dashes
		// i.e. /data/A/A/A --> -data-A-A-A
		name = name.replaceAll("/", "-");
		
		// If the file names starts with "-" or ends with "-" remove those
		// i.e. -data-A-A-A- --> data-A-A-A
		if (name.startsWith("-")) {
		...
		// Create the new file
		File path = new File(output_foldername + "/" + name);
		FileWriter fw = null;
		
		// Create the output stream if the name isn't empty (aka our first call to this method)
		if (!name.isEmpty()) {
			 fw = new FileWriter(path.getAbsolutePath());
		}
		
		// Traverse the files in the current folder
		for (File file : listOfFiles) {
			if (file.isFile()) {
				System.out.print(".");
				// If it is a file, extract the song information
				extractSongInfo(file.getAbsolutePath(), fw);
			} else {
				System.out.print("-");
				// If it is a folder add it to the recursive call
				browseFolder(file.getAbsolutePath(), output_foldername);
			}
		}
		
		// If the name isn't empty (aka we made a file writer) then flush and close the file writer
		if (!name.isEmpty()) {
			fw.flush();
			fw.close();
			
			// If the length of the file is 0 (it is empty) then delete it
			if (path.length() == 0) {
				path.delete();
			}
		}
